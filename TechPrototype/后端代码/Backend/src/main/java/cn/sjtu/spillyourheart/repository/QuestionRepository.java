...
public interface QuestionRepository extends JpaRepository<Question,Integer> {
    ...
    @Query(nativeQuery = true,value = "select * from Question as q where status = 1 and (?1=-1 or question_id<?1) and(?2='' or (header like ?2) or (content like ?2))and (?3=-1 or theme_id=?3) order by (select count(*) from answer as a where a.question_id = q.question_id) desc LIMIT 10")
    ...
    @Query(nativeQuery = true,value = "select * from Question where (status = 1 or status = -1) and (?1=-1 or question_id<?1) order by question_id desc limit 10")
    List<Question> getNotDeletedQuestions(int beginQuestionId);
}
