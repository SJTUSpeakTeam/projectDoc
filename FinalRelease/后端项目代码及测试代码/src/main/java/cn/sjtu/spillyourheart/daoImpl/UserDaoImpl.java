...
public class UserDaoImpl implements UserDao {
    static public Integer pageSize = 8;
    ...
    @Override
    public List<MetaUser> findAllMeta() {
        List<UserAuth> userAuths = userAuthRepository.findAll();
        List<MetaUser> metaUsers = new ArrayList<>();
        for(UserAuth userAuth: userAuths){
            metaUsers.add(findOneMeta(userAuth.getUserId()));
        }
    ...
    @Override
    public List<Relation> getCollectRelations(Integer beginTargetId,Integer userId,String area){
        //PageRequest request = new PageRequest(0, 10, Sort.Direction.DESC, "targetId");

        if(beginTargetId!=-1)
            return relationRepository.findCollectRelations(beginTargetId,userId,area,PageRequest.of(0, 10, Sort.by(Sort.Direction.DESC, "targetId")));
        return relationRepository.findAllCollectRelations(userId,area,PageRequest.of(0, 10,Sort.by(Sort.Direction.DESC, "targetId")));
    }
