...
public class QuestionServiceImpl implements QuestionService{
    ...
    @Override
    public Msg getNotDeletedQuestions(JSONObject object){
        ...
        if(object.containsKey("delete") && object.getBoolean("delete")){
            ...
            questionList = questionDao.getDeletedList(questionId,searchList,themeId);
        }
        ...
        if(object.containsKey("ban") && object.getBoolean("ban")){
            ...
            questionList.addAll(questionDao.getBannedList(questionId,searchList,themeId));
        }
        ...
        if(!getOrNot){
            ...
            questionList = questionDao.getAllList(questionId,searchList,themeId);
        }

        //将questionList重新排序
        Collections.sort(questionList, new Comparator<JSONObject>() {
            @Override
            public int compare(JSONObject o1, JSONObject o2) {
                return  o2.getInt("questionId")-o1.getInt("questionId");
            }
        });
    ...
    @Override
    public Msg getFilterQuestions(JSONObject object){
        if(!object.containsKey("beginQuestionId"))
            return MsgUtil.makeMsg(MsgCode.PARAMETER_ERROR);
        int questionId = -1;
        String searchList = "";
    ...
    @Override
    public Msg getUserAnswerQuestions(Integer beginQuestionId,Integer userId){
        JSONObject result = new JSONObject();
        List<Answer> answerList = answerDao.getUserAnswersOrderByQuestionId(beginQuestionId,userId);
        List<JSONObject> questionList = new ArrayList<>();
        for(Answer answer:answerList){
            ...
            if(questionDao.hasOne(questionId)){
                if(questionDao.verifyOne(questionId)){
                    if(!questionDao.bannedOne(questionId)){
                        object = questionDao.getOne(questionId);
                        object.remove("status");
                    }
                    else{
                        object.put("questionId",questionId);
                        object.put("answerId",answer.getAnswerId());
                        object.put("errorMessage","此问题已被封禁");
                    }
                }
                else{
                    object.put("questionId",questionId);
                    object.put("answerId",answer.getAnswerId());
                    object.put("errorMessage","此问题已被删除");
                }
            }
            else{
                object.put("questionId",questionId);
                object.put("answerId",answer.getAnswerId());
                object.put("errorMessage","此问题不存在");
            }
    ...
    @Override
    public Msg getUserCollectQuestions(Integer beginQuestionId,Integer userId){
        JSONObject result = new JSONObject();
        List<Relation> relationList = userDao.getCollectRelations(beginQuestionId,userId,"Question");
        List<JSONObject> questionList = new ArrayList<>();
        //get all questions
        for(Relation relation:relationList){
            System.out.println(relation.getTargetId());
            if(questionDao.hasOne(relation.getTargetId())
                    && questionDao.verifyOne(relation.getTargetId()))
                questionList.add(questionDao.getOne(relation.getTargetId()));
        }
        List<JSONObject> objectList = new ArrayList<>();
        for(JSONObject o:questionList){
            Integer _themeId = o.getInt("themeId");
    ...
    public Msg banQuestion(JSONObject object){
        ...
        if(!questionDao.hasOne(questionId))
            return MsgUtil.makeMsg(MsgCode.ERROR,"不存在相应提问 :(");
        if(questionDao.deletedOne(questionId))
            return MsgUtil.makeMsg(MsgCode.ERROR,"提问已被删除 :(");
        if(questionDao.bannedOne(questionId))
